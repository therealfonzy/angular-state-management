...
export const sectionStore = signalStore(
  ...
  withComputed((store) => {
    return {
      getFolders: computed(() => {
        ...
        console.log({ homeDocuments });
        const pathRoot = store.originalPath().split('/')[1];
        ...
        console.log({ homeFolder, otherFolders, folders: [homeFolder, ...otherFolders] });
  ...
  withHooks({
    onInit(store) {
      ...
      console.log('URL', routerState.routerState.snapshot.url);
