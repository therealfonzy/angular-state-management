...
import type { AppRoute } from '@app-shell/application/app-routing';
@Component({
  ...
  template: `
    ...
    >
      <span class="flex flex-row gap-4 items-start p-2">
        <div
          class="flex flex-col is-drawer-close:tooltip tooltip-top tooltip-primary "
          [attr.data-tip]="route.shortToolTip"
        >
          <ng-icon [name]="route.data.iconName" size="24px"></ng-icon>
          <div class="is-drawer-close:hidden flex flex-col ">
            <span>
              {{ route.data.linkText }}
